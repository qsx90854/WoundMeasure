...
def fit_reference_plane_from_shared_patterns(
        corners_left, corners_right, K_left, K_right, R_left_to_right,
        t_left_to_right, min_pattern_count=2):
    """Triangulate every corner of shared patterns and fit one reference plane.

    ``corners_left`` is frame B / the UI-left image and ``corners_right`` is
    frame A / the UI-right image.  The returned plane is therefore expressed
    in the UI-left camera frame, matching downstream wound-depth points.
    """
    if not isinstance(corners_left, dict) or not isinstance(corners_right, dict):
        return None

    shared_ids = sorted(set(corners_left).intersection(corners_right))
    ...
    R_left_to_right = np.asarray(R_left_to_right, dtype=np.float64).reshape(3, 3)
    t_left_to_right = np.asarray(t_left_to_right, dtype=np.float64).reshape(3, 1)
    P_left = K_left @ np.hstack([np.eye(3), np.zeros((3, 1))])
    P_right = K_right @ np.hstack([R_left_to_right, t_left_to_right])

    points_by_pattern = {}
    for marker_id in shared_ids:
        pts_left = np.asarray(corners_left[marker_id], dtype=np.float64).reshape(-1, 2)
        pts_right = np.asarray(corners_right[marker_id], dtype=np.float64).reshape(-1, 2)
        if pts_left.shape != (4, 2) or pts_right.shape != (4, 2):
            continue
        if not np.all(np.isfinite(pts_left)) or not np.all(np.isfinite(pts_right)):
            continue

        points_h = cv2.triangulatePoints(
            P_left, P_right, pts_left.T, pts_right.T)
        homogeneous_scale = points_h[3]
        if np.any(np.abs(homogeneous_scale) < 1e-12):
            continue
        points_left_3d = (points_h[:3] / homogeneous_scale).T
        points_right_3d = (
            R_left_to_right @ points_left_3d.T + t_left_to_right).T
        if (not np.all(np.isfinite(points_left_3d))
                or np.any(points_left_3d[:, 2] <= 0.0)
                or np.any(points_right_3d[:, 2] <= 0.0)):
            continue
        points_by_pattern[int(marker_id)] = points_left_3d

    if len(points_by_pattern) < int(min_pattern_count):
        return None

    plane_points = np.vstack(list(points_by_pattern.values()))
    plane_center = np.mean(plane_points, axis=0)
    _, singular_values, Vt = np.linalg.svd(plane_points - plane_center)
    if len(singular_values) < 2 or singular_values[1] <= 1e-9:
        return None
    plane_normal = Vt[-1]
    if np.dot(plane_normal, plane_center) > 0:
        plane_normal = -plane_normal
    residuals = (plane_points - plane_center) @ plane_normal
    rms_mm = float(np.sqrt(np.mean(residuals ** 2)))
    used_ids = sorted(points_by_pattern)
    log_and_print(
        f"✅ [多Pattern基準平面] IDs={used_ids}, 角點數={len(plane_points)}, "
        f"RMS={rms_mm:.3f} mm")
    return (
        plane_normal.astype(np.float64),
        plane_center.astype(np.float64),
        used_ids,
        rms_mm,
    )
...
def main():
    ...
    if multi_pattern_plane is not None:
        global_plane_n, global_plane_c, plane_pattern_ids, plane_rms_mm = multi_pattern_plane
        video_data['global_plane_n'] = global_plane_n
        video_data['global_plane_c'] = global_plane_c
        video_data['global_plane_marker_ids'] = plane_pattern_ids
    ...
    def on_export_circle_heights_csv(event):
        ...
        #   Sheet 2: Height_abs_mm_4x6   -> 4(row) x 6(col) 的 height_abs_mm 純數值矩陣
        # 第二頁第 1 列是 R1C1~R1C6 標題；第 2~5 列只放 height_abs_mm 數值。
        xlsx_saved = False
        xlsx_error = None
        try:
            ...
            # Sheet 2: header + 4x6 numeric matrix.
            # Row 1 is only the requested labels R1C1~R1C6.
            # Rows 2~5 contain only numeric height_abs_mm values for R1~R4.
            ...
            for c in range(1, int(CIRCLE_GRID_COLS) + 1):
                cell = ws_grid.cell(row=1, column=c)
                cell.value = f'R1C{c}'
                cell.font = header_font
                cell.fill = header_fill
            ...
            for c in range(1, int(CIRCLE_GRID_COLS) + 1):
                ws_grid.column_dimensions[ws_grid.cell(1, c).column_letter].width = 14
            ws_grid.row_dimensions[1].height = 24
            for r in range(2, int(CIRCLE_GRID_ROWS) + 2):
                ws_grid.row_dimensions[r].height = 24
            ws_grid.freeze_panes = 'A2'
