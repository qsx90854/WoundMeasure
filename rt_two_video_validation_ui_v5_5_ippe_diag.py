...
UI_BUILD = "2026-08-19 fixed-camera-v5.5-post-scale-corner-crosshair"
VIDEO_FILE_TYPES = [
...
def draw_marker_overlay_post_scale(
        shown: np.ndarray, corners: Any, caption: str, *,
        source_x0: float, source_y0: float,
        source_crop_w: float, source_crop_h: float) -> np.ndarray:
    """Draw corner diagnostics after crop/resize so marks stay thin at every zoom.

    Corners are transformed from full-frame coordinates into the already resized
    display image.  Each corner is a 1-pixel '+' crosshair with a small centre
    gap so the actual subpixel corner location is not painted over.
    """
    ...
    for marker_id, points_f in _iter_marker_corners(corners):
        ...
        mapped = np.empty_like(points_f, dtype=np.float64)
        mapped[:, 0] = (points_f[:, 0] - float(source_x0)) * sx
        mapped[:, 1] = (points_f[:, 1] - float(source_y0)) * sy

        visible_points = []
        for corner_index, (xf, yf) in enumerate(mapped):
            x = int(round(float(xf)))
            y = int(round(float(yf)))
            if x < -arm or x >= shown_w + arm or y < -arm or y >= shown_h + arm:
                continue
            visible_points.append((x, y))
            # Thin '+' crosshair.  Keep the centre pixels clear so the exact
            # detected corner remains visible even at high zoom.
            cv2.line(output, (x - arm, y), (x - gap, y), colour, 1, cv2.LINE_AA)
            cv2.line(output, (x + gap, y), (x + arm, y), colour, 1, cv2.LINE_AA)
            cv2.line(output, (x, y - arm), (x, y - gap), colour, 1, cv2.LINE_AA)
            cv2.line(output, (x, y + gap), (x, y + arm), colour, 1, cv2.LINE_AA)
            cv2.putText(
    ...
    # Caption is also drawn after resize so its thickness does not grow on zoom.
    cv2.rectangle(output, (0, 0), (output.shape[1], 30), (20, 20, 20), -1)
    cv2.putText(
        output, f"{caption} | markers: {marker_count}",
...
class TwoVideoRTValidationUI(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        self.display_detection_image_var = tk.BooleanVar(value=False)
        ...
        self._display_overlays: tuple[tuple[Any, str], tuple[Any, str]] | None = None
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(
            options, text="顯示 ArUco 偵測輸入",
            variable=self.display_detection_image_var,
            command=self._refresh_display_mode,
        ).grid(row=0, column=13, padx=(10, 5))
    ...
    def _start_analysis(self) -> None:
        ...
        self._append_log(
            ...
            f"ArUco_CLAHE={inputs['aruco_use_clahe']} | SIFT=disabled"
        )
    ...
    def _refresh_display_mode(self) -> None:
        ...
        try:
            ...
            show_detection = bool(self.display_detection_image_var.get())
            ...
            if show_detection:
                ...
                caption_a = f"Video A | frame {index_a} | ArUco input: {mode}"
                caption_b = f"Video B | frame {index_b} | ArUco input: {mode}"
            else:
            ...
            self._display_overlays = (
                (result.get("corners_a"), caption_a),
                (result.get("corners_b"), caption_b),
            )
            self._zoom_factors = [1.0, 1.0]
            self._view_centers = [None, None]
            self._drag_last = [None, None]
    ...
    def _render_selected_frames(self) -> None:
        ...
        for slot, (canvas, frame) in enumerate(
            ...
            if self._display_overlays is not None:
                overlay_corners, overlay_caption = self._display_overlays[slot]
                shown = draw_marker_overlay_post_scale(
                    shown, overlay_corners, overlay_caption,
