"""
depth_measure_multi_aruco_sbs_camera_v7_demo_SU01.py
================
SU01 雙目相機互動式深度量測工具（可調 ROI 版）。

特點：
- 支援即時錄製 SU01 雙目影片或載入既有雙目影片
...
END_FRAME_COUNT       = 1                          # 固定影像對左圖
FRAME_RANGE_MODE      = "half_half"                # 兩張固定影像會分到前、後半段
...
CAMERA_WIDTH          = 2560                       # SU01 SBS 完整影像寬（左右各 1280）
CAMERA_HEIGHT         = 1024                       # SU01 SBS 完整影像高
CAMERA_PREVIEW_MAX_WIDTH = 960                     # 僅縮小預覽；不影響錄影解析度
# 左右單眼各自的正規化 ROI；需滿足 X+WIDTH<=1、Y+HEIGHT<=1。
# 目前兩眼先沿用原本共用設定 X=0.5、Y=0.25、W=0.5、H=0.5。
RT_ROI_LEFT_X_RATIO      = 0.5
RT_ROI_LEFT_Y_RATIO      = 0.25
RT_ROI_LEFT_WIDTH_RATIO  = 0.5
RT_ROI_LEFT_HEIGHT_RATIO = 0.5
RT_ROI_RIGHT_X_RATIO      = 0.0
RT_ROI_RIGHT_Y_RATIO      = 0.25
RT_ROI_RIGHT_WIDTH_RATIO  = 0.5
RT_ROI_RIGHT_HEIGHT_RATIO = 0.5

PARAMS_JSON_PATH      = "calibration_result_su01.json"
...
def load_su01_calibration(json_path):
    """Load intrinsics and JSON extrinsics used by the all-frame selector."""
    path = Path(json_path)
    if not path.is_absolute():
        path = BASE_DIR / path
    if not path.exists():
        raise FileNotFoundError(f"找不到相機參數檔案: {path}")

    with path.open("r", encoding="utf-8") as file:
        data = json.load(file)

    try:
        intrinsic_left = data["intrinsic_L"]
        intrinsic_right = data["intrinsic_R"]
        mtx_left = np.asarray(intrinsic_left["matrix"], dtype=np.float64).reshape(3, 3)
        dist_left = np.asarray(intrinsic_left["distortion"], dtype=np.float64).reshape(-1)
        mtx_right = np.asarray(intrinsic_right["matrix"], dtype=np.float64).reshape(3, 3)
        dist_right = np.asarray(intrinsic_right["distortion"], dtype=np.float64).reshape(-1)
    except (KeyError, TypeError, ValueError) as exc:
        raise ValueError(f"相機參數 JSON 格式不完整: {path}") from exc

    answer_extrinsic = data.get("extrinsic", {})
    log_and_print(f"✅ 已載入 SU01 左右內參: {path}")
    log_and_print(
...
def read_sbs_reference_frame(video_path, frame_index):
    """Read one zero-based frame and split it into left/right camera images."""
    ...
    try:
        ...
        if total_frames > 0 and frame_index >= total_frames:
            raise IndexError(
                f"影片只有 {total_frames} 幀，無法讀取 F{frame_index}。"
                "請錄製至少 51 幀。"
            )
    ...
    if not ok or sbs_frame is None:
        raise OSError(f"無法解碼雙目影片的 F{frame_index}: {video_path}")

    height, full_width = sbs_frame.shape[:2]
    if full_width % 2:
        raise ValueError(f"雙目影像寬度必須為偶數，目前為 {full_width}")
    if (full_width, height) != (CAMERA_WIDTH, CAMERA_HEIGHT):
        raise ValueError(
            f"影片解析度為 {full_width}x{height}，但 SU01 標定與程式設定為 "
            f"{CAMERA_WIDTH}x{CAMERA_HEIGHT}；為避免內參尺度錯誤，已停止解算。"
        )

    half_width = full_width // 2
    left_raw = sbs_frame[:, :half_width].copy()
    right_raw = sbs_frame[:, half_width:].copy()
    log_and_print(
        f"🎯 固定使用 F{frame_index}：SBS={full_width}x{height}，"
        f"Left={left_raw.shape[1]}x{left_raw.shape[0]}，"
        f"Right={right_raw.shape[1]}x{right_raw.shape[0]}"
    )
    return left_raw, right_raw, total_frames


def map_stereo_pair_to_common_intrinsic(
    left_raw,
    right_raw,
    mtx_left,
    dist_left,
    ...
    """Undistort each camera with its own calibration into one common pixel model."""
    if left_raw.shape[:2] != right_raw.shape[:2]:
        raise ValueError(
            f"左右影像尺寸不同: left={left_raw.shape[:2]}, right={right_raw.shape[:2]}"
        )

    height, width = left_raw.shape[:2]
    image_size = (width, height)
    common_k, _ = cv2.getOptimalNewCameraMatrix(
        mtx_left, dist_left, image_size, 1.0, image_size
    )
    map_left_1, map_left_2 = cv2.initUndistortRectifyMap(
        mtx_left, dist_left, None, common_k, image_size, cv2.CV_16SC2
    )
    map_right_1, map_right_2 = cv2.initUndistortRectifyMap(
        mtx_right, dist_right, None, common_k, image_size, cv2.CV_16SC2
    )
    left_common = cv2.remap(
        left_raw, map_left_1, map_left_2, cv2.INTER_LINEAR
    )
    right_common = cv2.remap(
        right_raw, map_right_1, map_right_2, cv2.INTER_LINEAR
    )
    return left_common, right_common, common_k.astype(np.float64)
...
def compare_rt_with_json_answer(R_est, t_est, answer_extrinsic):
    ...
    if "R" not in answer_extrinsic or "T" not in answer_extrinsic:
        log_and_print("⚠️ JSON extrinsic 缺少 R 或 T，略過對答案。")
        return None
    ...
    try:
        ...
        log_and_print(f"⚠️ JSON extrinsic 格式錯誤，略過對答案: {exc}")
        return None
    ...
    baseline_answer = float(np.linalg.norm(t_answer))
    baseline_error = baseline_est - baseline_answer
    translation_l2_error = float(np.linalg.norm(t_est - t_answer))

    if baseline_est > 1e-9 and baseline_answer > 1e-9:
        direction_cos = np.clip(
            float((t_est.T @ t_answer)[0, 0]) / (baseline_est * baseline_answer),
            -1.0,
            1.0,
        )
    ...
    result = {
        ...
        "translation_l2_error_mm": translation_l2_error,
        "translation_direction_error_deg": translation_direction_error_deg,
        "estimated_baseline_mm": baseline_est,
    ...
    log_and_print(
        ...
        f"direction error: {translation_direction_error_deg:.4f} deg"
    )
    log_and_print(
        f"Baseline: algorithm={baseline_est:.4f} mm | "
        f"JSON={baseline_answer:.4f} mm | delta={baseline_error:+.4f} mm"
    )
    log_and_print("====================================================")
    return result
...
def find_best_sbs_frame_by_json(
    ...
    def analyze_pair(right_common, left_common, frame_index, common_k):
...
def record_video_from_camera():
    ...
    while True:
        ...
        # 縮小預覽以降低 4K SBS 畫面的 UI 負擔；錄影仍保存完整解析度。
        preview_scale = min(1.0, CAMERA_PREVIEW_MAX_WIDTH / float(w))
        display_small = cv2.resize(
            display_frame,
            (int(round(w * preview_scale)), int(round(h * preview_scale))),
...
def analyze_video_with_progress_bar(
    video_path,
    start_n,
    end_n,
    K_L,
...
def main():
    ...
    try:
        mtxL_raw, distL_raw, mtxR_raw, distR_raw, answer_extrinsic = (
            load_su01_calibration(PARAMS_JSON_PATH)
        )
        ...
        print(f"❌ SU01 雙目初始化失敗: {exc}")
        sys.exit(1)
    except RuntimeError as exc:
        print(f"❌ SU01 全幀 RT 搜尋失敗: {exc}")
        sys.exit(1)

    # 後續原演算法在共同的去畸變座標工作，因此有效畸變為零。
    distL = np.zeros(5, dtype=np.float64)
    mtxL_o = KL.copy()
    h_raw, w_alg = left_common.shape[:2]
    active_u = w_alg // 2
    active_v = h_raw // 2

    def process_view(image, *_args, **_kwargs):
    ...
    current_cand = {
        ...
        'pose_info': (
            f"SU01 F{video_data.get('source_frame_index', '?')} SBS algorithm RT "
