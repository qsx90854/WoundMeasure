...
CAMERA_HEIGHT         = 1440                        # 相機解析度高


PARAMS_JSON_PATH      = "calibration_result_c2.json"  # 標定參數 JSON 檔路徑
...
ENABLE_POSE_SMOOTHING = True                          # 是否啟用時序平滑濾波
...
def main():
    ...
    print(f"📷 實際讀取相機解析度為: {w_raw} x {h_raw}")
    w_alg = w_raw # 使用全影像
    active_u = w_alg // 2
    active_v = h_raw // 2
        ...
        if m_pt is None:
            ...
            if m_pt is None and snap_view_state['grad_sift']:
                ...
                if patch_g.size > 0:
                    ...
                    rad = max(20, int(round(20 * (KL[0, 0] / 650.0)))); uei, vei = int(round(u_exp)), int(round(v_exp))
    ...
    def update_display(avg, summary):
        ...
        if res['pt'] is not None:
            ...
            if res['d'] is not None:
                ...
                main_text = (f"深度值(距離): {res['d']:.1f}mm\n"
                             f"相機3D座標: {c_coords_str}\n"
        ...
        while True:
            ...
            try:
                ...
                if len(all_R_r) > 0:
                    ...
                    # 🌟 修正平面擬合：直接以左圖單獨估出的 3D 點 objA 來進行平面擬合，避免依賴相對位姿，徹底切斷污染源
                    all_pts3d = []
                    for objA_i in all_objA:
                        for p in objA_i:
                            all_pts3d.append(p)
                    if len(all_pts3d) >= 3:
