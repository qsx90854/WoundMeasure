...
MIN_POSE_BASELINE_MM  = 5.0                        # 原始位姿估計 Baseline 下限 (mm)
MAX_POSE_BASELINE_MM  = 500.0                       # 原始位姿估計 Baseline 上限 (mm)
...
def compute_reprojection_debug_data(cA_dict, cB_dict, curr_marker_poses, R_rel, t_rel, K_L, K_R, marker_size_mm):
    ...
    for mid in cA_dict.keys():
        if mid not in cB_dict or mid not in curr_marker_poses:
            continue
        rv_L, tv_L = curr_marker_poses[mid]
        R_L, _ = cv2.Rodrigues(rv_L)
        tv_L = tv_L.reshape(3, 1)
        
        # 左相機空間 3D 點
        pts_3D_L = (R_L @ canon.T + tv_L).T
        
        # 投影至右相機 (B)
        pts_3D_R = (R_rel @ pts_3D_L.T + t_rel.reshape(3, 1)).T
        pts_proj_B, _ = cv2.projectPoints(pts_3D_R, np.zeros(3), np.zeros(3), K_R, np.zeros(5))
        pts_proj_B = pts_proj_B.reshape(4, 2)
        proj_ptsA_on_B[mid] = pts_proj_B
        
        # 右圖重投影誤差
        err_B = np.linalg.norm(pts_proj_B - cB_dict[mid], axis=1)
        errors_B.extend(err_B)
        
        # 投影至左相機 (A) - 使用相對外參的逆變換
        pts_3D_L_back = (R_rel.T @ (pts_3D_R.T - t_rel.reshape(3, 1))).T
        pts_proj_A, _ = cv2.projectPoints(pts_3D_L_back, np.zeros(3), np.zeros(3), K_L, np.zeros(5))
        pts_proj_A = pts_proj_A.reshape(4, 2)
        proj_ptsB_on_A[mid] = pts_proj_A
        
        # 左圖重投影誤差
        err_A = np.linalg.norm(pts_proj_A - cA_dict[mid], axis=1)
        errors_A.extend(err_A)
        
    all_errors = errors_A + errors_B
    mean_err = np.mean(all_errors) if all_errors else 0.0
...
def main():
    ...
    def draw_projected_aruco(ax, proj_corners, color='red'):
        ...
        ax.proj_art = []
        if not proj_corners: return
        for mid, pts in proj_corners.items():
            p = np.vstack((pts, pts[0]))
            l, = ax.plot(p[:, 0], p[:, 1], color=color, lw=1.5, linestyle='--')
            center = np.mean(pts, axis=0)
            t = ax.text(center[0], center[1], f"Proj:{mid}", color=color, fontsize=8, fontweight='bold', ha='center', va='center', zorder=4)
            ax.proj_art.extend([l, t])
        ...
        while True:
            ...
            try:
                ...
                if res_pose is not None:
                    ...
                    if not (MIN_POSE_BASELINE_MM <= raw_baseline <= MAX_POSE_BASELINE_MM):
                        print(f"❌ 算出的原始 Baseline 異常 ({raw_baseline:.2f} mm，合理區間: {MIN_POSE_BASELINE_MM}~{MAX_POSE_BASELINE_MM} mm)，判定為無視差退化或定位異常，忽略此幀")
                        res_pose = None
