...
# ---- Localized Plane-Sweep / PatchMatch-style single-point matcher ----
# Marker/global plane is used only to center the search range.  The actual point
# is allowed to move away from that plane by sweeping depth hypotheses and local
# surface-normal hypotheses.  Geometry handles rotation / scale / perspective;
# robust photometric scoring only evaluates the already-warped appearance.
LOCAL_PS_DEPTH_RANGE_MM = 45.0
LOCAL_PS_COARSE_SAMPLES = 31
LOCAL_PS_FINE_SAMPLES = 13
LOCAL_PS_PATCH_SIZE = 31
LOCAL_PS_NORMAL_TILT_DEG = 12.0
LOCAL_PS_FINE_NORMAL_TILT_DEG = 3.0
LOCAL_PS_MIN_SCORE = 0.44
LOCAL_PS_MIN_VALID_RATIO = 0.50
LOCAL_PS_MIN_SUPPORT_RATIO = 0.35
LOCAL_PS_MIN_UNIQUENESS = 0.035
LOCAL_PS_FB_RANGE_MM = 7.0
LOCAL_PS_FB_SAMPLES = 11
LOCAL_PS_FB_SIGMA_PX = 1.5
LOCAL_PS_MIN_DEPTH_MM = 50.0
LOCAL_PS_MAX_DEPTH_MM = 2000.0
LOCAL_PS_OCCLUSION_TRIM_FRACTION = 0.20
...
TUNABLE_CONSTANTS = [
    ...
    'LOCAL_PS_DEPTH_RANGE_MM',
    'LOCAL_PS_COARSE_SAMPLES',
    'LOCAL_PS_FINE_SAMPLES',
    'LOCAL_PS_PATCH_SIZE',
    'LOCAL_PS_NORMAL_TILT_DEG',
    'LOCAL_PS_FINE_NORMAL_TILT_DEG',
    'LOCAL_PS_MIN_SCORE',
    'LOCAL_PS_MIN_VALID_RATIO',
    'LOCAL_PS_MIN_SUPPORT_RATIO',
    'LOCAL_PS_MIN_UNIQUENESS',
    'LOCAL_PS_FB_RANGE_MM',
    'LOCAL_PS_FB_SAMPLES',
    'LOCAL_PS_FB_SIGMA_PX',
    'LOCAL_PS_MIN_DEPTH_MM',
    'LOCAL_PS_MAX_DEPTH_MM',
    'LOCAL_PS_OCCLUSION_TRIM_FRACTION',
    'LOCAL_PS_ENABLE_FB',
]
...
def _normalize_vec(v, fallback=None):
    v = np.asarray(v, dtype=np.float64).reshape(-1)
    n = float(np.linalg.norm(v))
    if n < 1e-12:
        if fallback is None:
            return None
        v = np.asarray(fallback, dtype=np.float64).reshape(-1)
        n = max(float(np.linalg.norm(v)), 1e-12)
    return v / n


def _camera_ray_from_pixel(pt, K):
    ray = np.linalg.inv(np.asarray(K, dtype=np.float64)) @ np.array([float(pt[0]), float(pt[1]), 1.0], dtype=np.float64)
    if abs(float(ray[2])) < 1e-12:
        return None
    return ray / float(ray[2])


def _estimate_reference_depth_from_plane(pt, K, plane_n, plane_c):
    """Return camera-Z where the clicked ray intersects the marker/global plane."""
    if plane_n is None or plane_c is None:
        return None
    ray = _camera_ray_from_pixel(pt, K)
    if ray is None:
        return None
    n = _normalize_vec(plane_n)
    c = np.asarray(plane_c, dtype=np.float64).reshape(3)
    denom = float(np.dot(n, ray))
    d = float(np.dot(n, c))
    if abs(denom) < 1e-9:
        return None
    z = d / denom  # ray is normalized to ray[2] == 1, so scale == camera Z
    if not np.isfinite(z) or z <= 0:
        return None
    return float(z)


def _normal_tangent_basis(n):
    n = _normalize_vec(n, fallback=[0.0, 0.0, 1.0])
    helper = np.array([0.0, 1.0, 0.0], dtype=np.float64)
    if abs(float(np.dot(helper, n))) > 0.90:
        helper = np.array([1.0, 0.0, 0.0], dtype=np.float64)
    t1 = _normalize_vec(np.cross(helper, n))
    t2 = _normalize_vec(np.cross(n, t1))
    return n, t1, t2


def _tilt_normal(n, direction, angle_deg):
    n = _normalize_vec(n, fallback=[0.0, 0.0, 1.0])
    ...
    return _normalize_vec(np.cos(a) * n + np.sin(a) * d, fallback=n)


def _make_normal_hypotheses(base_n, tilt_deg, include_diagonal=True):
    n, t1, t2 = _normal_tangent_basis(base_n)
    out = [n]
    for sign in (-1.0, 1.0):
        out.append(_tilt_normal(n, t1, sign * tilt_deg))
        out.append(_tilt_normal(n, t2, sign * tilt_deg))
    if include_diagonal:
        diag_deg = float(tilt_deg) * 0.78
        for sx in (-1.0, 1.0):
            for sy in (-1.0, 1.0):
                d = _normalize_vec(sx * t1 + sy * t2)
                out.append(_tilt_normal(n, d, diag_deg))
    return out


def _plane_homography_for_hypothesis(pt_ref, depth_z, normal_ref, K_ref, K_src, R_ref_to_src, t_ref_to_src):
    ray = _camera_ray_from_pixel(pt_ref, K_ref)
    if ray is None:
        return None, None
    X = ray * float(depth_z)
    n = _normalize_vec(normal_ref, fallback=[0.0, 0.0, 1.0])
    d = float(np.dot(n, X))
    if abs(d) < 1e-8:
        return None, X
    R = np.asarray(R_ref_to_src, dtype=np.float64).reshape(3, 3)
    t = np.asarray(t_ref_to_src, dtype=np.float64).reshape(3, 1)
    H = np.asarray(K_src, dtype=np.float64) @ (R + (t @ n.reshape(1, 3)) / d) @ np.linalg.inv(np.asarray(K_ref, dtype=np.float64))
    return H, X


def _sample_patch(img, pt, patch_size, interpolation=cv2.INTER_LINEAR, border_value=0):
    half = patch_size // 2
    coords = np.arange(-half, half + 1, dtype=np.float32)
    dx, dy = np.meshgrid(coords, coords)
    map_x = (float(pt[0]) + dx).astype(np.float32)
    map_y = (float(pt[1]) + dy).astype(np.float32)
    h, w = img.shape[:2]
    valid = (map_x >= 0.0) & (map_x <= w - 1.0) & (map_y >= 0.0) & (map_y <= h - 1.0)
    patch = cv2.remap(img, map_x, map_y, interpolation=interpolation,
                      borderMode=cv2.BORDER_CONSTANT, borderValue=border_value)
    return patch, valid, map_x, map_y


def _warp_source_patch_to_reference_grid(src_img, H_ref_to_src, pt_ref, patch_size,
                                         src_exclude_mask=None):
    half = patch_size // 2
    coords = np.arange(-half, half + 1, dtype=np.float32)
    dx, dy = np.meshgrid(coords, coords)
    x_ref = float(pt_ref[0]) + dx
    y_ref = float(pt_ref[1]) + dy
    ones = np.ones_like(x_ref, dtype=np.float64)
    p = np.stack([x_ref.astype(np.float64), y_ref.astype(np.float64), ones], axis=0).reshape(3, -1)
    q = np.asarray(H_ref_to_src, dtype=np.float64) @ p
    qw = q[2]
    finite = np.isfinite(q).all(axis=0) & (np.abs(qw) > 1e-9)
    map_x = np.full(qw.shape, -1.0, dtype=np.float32)
    map_y = np.full(qw.shape, -1.0, dtype=np.float32)
    map_x[finite] = (q[0, finite] / qw[finite]).astype(np.float32)
    map_y[finite] = (q[1, finite] / qw[finite]).astype(np.float32)
    map_x = map_x.reshape(patch_size, patch_size)
    map_y = map_y.reshape(patch_size, patch_size)
    h, w = src_img.shape[:2]
    valid = finite.reshape(patch_size, patch_size)
    valid &= (map_x >= 0.0) & (map_x <= w - 1.0) & (map_y >= 0.0) & (map_y <= h - 1.0)
    warped = cv2.remap(src_img, map_x, map_y, interpolation=cv2.INTER_LINEAR,
                       borderMode=cv2.BORDER_CONSTANT, borderValue=0)
    if src_exclude_mask is not None:
        warped_mask = cv2.remap(src_exclude_mask.astype(np.uint8), map_x, map_y,
...
def _masked_ncc(a, b, mask):
    ...
    if np.count_nonzero(mask) < 9:
        return 0.0
    av = np.asarray(a, dtype=np.float32)[mask]
    bv = np.asarray(b, dtype=np.float32)[mask]
    av = av - float(av.mean())
    bv = bv - float(bv.mean())
...
def _standardize_for_residual(arr, mask):
    ...
    sd = float(vals.std())
    if sd < 1e-3:
        return np.zeros_like(arr, dtype=np.float32), sd
    return (np.asarray(arr, dtype=np.float32) - mu) / sd, sd


def _robust_warp_score(ref_patch, warped_src, valid_mask, trim_fraction=LOCAL_PS_OCCLUSION_TRIM_FRACTION):
    """
    Robust photometric score on already geometry-aligned patches.

    - ZNCC is evaluated only after the depth/normal homography has removed the
      expected scale/rotation/perspective deformation.
    - Pixel outliers are treated as local occlusion / non-Lambertian support loss,
      not as a reason to reject the whole candidate frame.
    """
    valid = np.asarray(valid_mask, dtype=bool)
    total = int(valid.size)
    n_valid = int(np.count_nonzero(valid))
    valid_ratio = n_valid / max(total, 1)
    if n_valid < max(25, int(total * 0.25)):
        return {'score': 0.0, 'valid_ratio': valid_ratio, 'support_ratio': 0.0,
                'occlusion_ratio': 1.0, 'center_visible': False,
                'intensity_ncc': 0.0, 'gradient_ncc': 0.0}

    A = np.asarray(ref_patch, dtype=np.float32)
    B = np.asarray(warped_src, dtype=np.float32)
    gA = cv2.magnitude(cv2.Sobel(A, cv2.CV_32F, 1, 0, ksize=3),
                       cv2.Sobel(A, cv2.CV_32F, 0, 1, ksize=3))
    gB = cv2.magnitude(cv2.Sobel(B, cv2.CV_32F, 1, 0, ksize=3),
                       cv2.Sobel(B, cv2.CV_32F, 0, 1, ksize=3))

    zA, sdA = _standardize_for_residual(A, valid)
    zB, sdB = _standardize_for_residual(B, valid)
    zgA, sdgA = _standardize_for_residual(gA, valid)
    zgB, sdgB = _standardize_for_residual(gB, valid)

    # Low-texture regions are expected in wounds.  If intensity has almost no
    # variance, do not let an unstable normalized residual dominate; lean on
    # whatever gradient/context is available instead.
    intensity_resid = np.abs(zA - zB) if min(sdA, sdB) > 1.0 else np.zeros_like(A, dtype=np.float32)
    gradient_resid = np.abs(zgA - zgB) if min(sdgA, sdgB) > 0.5 else np.zeros_like(A, dtype=np.float32)
    residual = intensity_resid + 0.45 * gradient_resid
    rv = residual[valid]
    med = float(np.median(rv))
    mad = float(np.median(np.abs(rv - med)))
    robust_thr = min(2.0, max(0.95, med + 2.5 * 1.4826 * mad))
    support = valid & (residual <= robust_thr)
    n_support = int(np.count_nonzero(support))
    if n_support < max(16, int(n_valid * 0.30)):
        # Keep the frame/candidate alive when the robust threshold is unstable;
        # confidence will still fall through the photometric score.
        support = valid.copy()
        n_support = n_valid

    # Visibility is estimated before optional trimming.  Therefore a clean patch
    # reports occlusion_ratio near zero instead of being artificially fixed to
    # the configured trim fraction.
    support_ratio = n_support / max(n_valid, 1)
    occlusion_ratio = float(np.clip(1.0 - support_ratio, 0.0, 1.0))

    # For the NCC itself, optionally trim only the worst residual tail *inside*
    # the visible support.  This improves robustness to a small occluded corner
    # without changing the visibility estimate above.
    score_mask = support.copy()
    trim_fraction = float(np.clip(trim_fraction, 0.0, 0.45))
    if trim_fraction > 0.0 and n_support >= 20:
        sv = residual[support]
        trim_thr = float(np.quantile(sv, 1.0 - trim_fraction))
        trimmed = support & (residual <= trim_thr)
        if np.count_nonzero(trimmed) >= max(16, int(n_support * 0.5)):
            score_mask = trimmed

    ncc_i = _masked_ncc(A, B, score_mask)
    ncc_g = _masked_ncc(gA, gB, score_mask)
    # Map [-1,1] NCC to [0,1].  Gradient carries substantial weight because
    # wound intensity can change with illumination while geometry-aligned edge
    # structure is often more stable.
    score_i = 0.5 * (ncc_i + 1.0)
    score_g = 0.5 * (ncc_g + 1.0)
    base_score = 0.55 * score_i + 0.45 * score_g
    score = float(np.clip(base_score * (0.78 + 0.22 * support_ratio), 0.0, 1.0))

    h, w = valid.shape
    r = max(2, min(h, w) // 8)  # about a 7x7 center for 31x31 patches
    cy, cx = h // 2, w // 2
    center_valid = valid[max(0, cy-r):min(h, cy+r+1), max(0, cx-r):min(w, cx+r+1)]
    center_support = support[max(0, cy-r):min(h, cy+r+1), max(0, cx-r):min(w, cx+r+1)]
    cv = int(np.count_nonzero(center_valid))
    cs = int(np.count_nonzero(center_support))
    center_visible = (cv >= max(5, center_valid.size // 3)) and (cs / max(cv, 1) >= 0.45)

    return {'score': score, 'valid_ratio': float(valid_ratio),
            'support_ratio': float(support_ratio), 'occlusion_ratio': occlusion_ratio,
            'center_visible': bool(center_visible),
            'intensity_ncc': float(ncc_i), 'gradient_ncc': float(ncc_g)}


def _project_hypothesis_center(pt_ref, depth_z, K_ref, K_src, R_ref_to_src, t_ref_to_src):
    ray = _camera_ray_from_pixel(pt_ref, K_ref)
    if ray is None:
        return None, None
    X_ref = ray * float(depth_z)
    X_src = np.asarray(R_ref_to_src, dtype=np.float64) @ X_ref.reshape(3, 1) + np.asarray(t_ref_to_src, dtype=np.float64).reshape(3, 1)
    X_src = X_src.reshape(3)
    if X_src[2] <= 1e-6:
        return None, X_src
    q = np.asarray(K_src, dtype=np.float64) @ X_src
    if abs(float(q[2])) < 1e-9:
        return None, X_src
    return np.array([q[0] / q[2], q[1] / q[2]], dtype=np.float32), X_src


def _evaluate_plane_hypothesis(ref_gray, src_gray, pt_ref, depth_z, normal_ref,
                               K_ref, K_src, R_ref_to_src, t_ref_to_src,
                               ref_patch, ref_valid, patch_size,
                               ref_exclude_patch=None, src_exclude_mask=None):
    H, X_ref = _plane_homography_for_hypothesis(pt_ref, depth_z, normal_ref,
                                                K_ref, K_src, R_ref_to_src, t_ref_to_src)
    if H is None:
        return None
    warped, src_valid, _mx, _my = _warp_source_patch_to_reference_grid(
        src_gray, H, pt_ref, patch_size, src_exclude_mask=src_exclude_mask)
    valid = ref_valid & src_valid
    if ref_exclude_patch is not None:
        valid &= ~ref_exclude_patch
    stats = _robust_warp_score(ref_patch, warped, valid)
    pt_src, X_src = _project_hypothesis_center(pt_ref, depth_z, K_ref, K_src,
                                               R_ref_to_src, t_ref_to_src)
    if pt_src is None:
        return None
    stats.update({'depth': float(depth_z), 'normal': np.asarray(normal_ref, dtype=np.float64),
                  'pt_src': pt_src, 'H': H, 'X_ref': X_ref, 'X_src': X_src})
    return stats


def _inverse_depth_samples(z_center, half_range_mm, count, z_min, z_max):
    z0 = max(float(z_min), float(z_center) - float(half_range_mm))
    z1 = min(float(z_max), float(z_center) + float(half_range_mm))
    if z1 <= z0 + 1e-6:
        return np.array([float(np.clip(z_center, z_min, z_max))], dtype=np.float64)
    rho0, rho1 = 1.0 / z1, 1.0 / z0
    rho = np.linspace(rho0, rho1, max(int(count), 3), dtype=np.float64)
    return 1.0 / rho


def _localized_plane_sweep_core(ref_gray, src_gray, pt_ref, K_ref, K_src,
                                R_ref_to_src, t_ref_to_src,
                                center_depth, base_normal,
                                ref_exclude_mask=None, src_exclude_mask=None,
                                depth_half_range=LOCAL_PS_DEPTH_RANGE_MM,
                                coarse_samples=LOCAL_PS_COARSE_SAMPLES,
                                fine_samples=LOCAL_PS_FINE_SAMPLES,
                                patch_size=LOCAL_PS_PATCH_SIZE,
                                normal_tilt_deg=LOCAL_PS_NORMAL_TILT_DEG,
                                fine_normal_tilt_deg=LOCAL_PS_FINE_NORMAL_TILT_DEG):
    if center_depth is None or not np.isfinite(center_depth) or center_depth <= 0:
        return {'m_pt': None, 'reject_reason': '無可用的初始深度範圍'}

    ref_patch, ref_valid, _rx, _ry = _sample_patch(ref_gray, pt_ref, patch_size)
    ref_exclude_patch = None
    if ref_exclude_mask is not None:
        ex_patch, ex_valid, _x, _y = _sample_patch(ref_exclude_mask.astype(np.uint8), pt_ref, patch_size,
                                                  interpolation=cv2.INTER_NEAREST, border_value=255)
        ref_exclude_patch = (ex_patch > 0) | (~ex_valid)
    if np.count_nonzero(ref_valid) < patch_size * patch_size * 0.5:
        return {'m_pt': None, 'reject_reason': '左圖 patch 超出影像範圍'}

    coarse_depths = _inverse_depth_samples(center_depth, depth_half_range, coarse_samples,
                                           LOCAL_PS_MIN_DEPTH_MM, LOCAL_PS_MAX_DEPTH_MM)
    normals = _make_normal_hypotheses(base_normal, normal_tilt_deg, include_diagonal=True)
    coarse_records = []
    for z in coarse_depths:
        for n in normals:
            rec = _evaluate_plane_hypothesis(ref_gray, src_gray, pt_ref, z, n,
                                             K_ref, K_src, R_ref_to_src, t_ref_to_src,
                                             ref_patch, ref_valid, patch_size,
                                             ref_exclude_patch, src_exclude_mask)
            if rec is not None:
                coarse_records.append(rec)
    if not coarse_records:
        return {'m_pt': None, 'reject_reason': '深度掃描沒有有效幾何假設'}
    coarse_best = max(coarse_records, key=lambda r: r['score'])

    # Refine only around the best inverse-depth neighborhood, while allowing a
    # small local-normal adjustment around the coarse winner.
    cidx = int(np.argmin(np.abs(coarse_depths - coarse_best['depth'])))
    lo = max(0, cidx - 1)
    hi = min(len(coarse_depths) - 1, cidx + 1)
    z_lo = float(min(coarse_depths[lo], coarse_depths[hi]))
    z_hi = float(max(coarse_depths[lo], coarse_depths[hi]))
    if hi == lo:
        z_lo = max(LOCAL_PS_MIN_DEPTH_MM, coarse_best['depth'] - 2.0)
        z_hi = min(LOCAL_PS_MAX_DEPTH_MM, coarse_best['depth'] + 2.0)
    rho = np.linspace(1.0 / z_hi, 1.0 / z_lo, max(int(fine_samples), 5))
    fine_depths = 1.0 / rho
    fine_normals = _make_normal_hypotheses(coarse_best['normal'], fine_normal_tilt_deg, include_diagonal=False)
    fine_records = []
    for z in fine_depths:
        for n in fine_normals:
            rec = _evaluate_plane_hypothesis(ref_gray, src_gray, pt_ref, z, n,
                                             K_ref, K_src, R_ref_to_src, t_ref_to_src,
                                             ref_patch, ref_valid, patch_size,
                                             ref_exclude_patch, src_exclude_mask)
            if rec is not None:
                fine_records.append(rec)
    records = fine_records if fine_records else coarse_records
    best = max(records, key=lambda r: r['score'])

    # Build a depth score curve (maximize over normal at each depth).  The second
    # peak excludes the immediate neighbor of the best depth so a naturally broad
    # correlation peak is not mistaken for a second independent match.
    depth_values = fine_depths if fine_records else coarse_depths
    curve = []
    best_by_depth = []
    for z in depth_values:
        same = [r for r in records if abs(r['depth'] - float(z)) < 1e-6]
        if same:
            rb = max(same, key=lambda r: r['score'])
            curve.append(float(rb['score']))
            best_by_depth.append(rb)
        else:
            curve.append(-1.0)
            best_by_depth.append(None)
    curve = np.asarray(curve, dtype=np.float64)
    ib = int(np.argmax(curve))
    second_candidates = [float(curve[i]) for i in range(len(curve)) if abs(i - ib) > 1 and np.isfinite(curve[i])]
    second_score = max(second_candidates) if second_candidates else max(0.0, float(best['score']) - 0.05)
    uniqueness = float(max(0.0, float(best['score']) - second_score))
    boundary_hit = bool(ib == 0 or ib == len(curve) - 1)

    # Sub-depth refinement: parabola in inverse depth using the best normal at
    # the discrete optimum.  Re-evaluate the geometry at the refined depth.
    if 0 < ib < len(curve) - 1 and curve[ib-1] >= 0 and curve[ib+1] >= 0:
        rr = 1.0 / np.asarray([depth_values[ib-1], depth_values[ib], depth_values[ib+1]], dtype=np.float64)
        ss = np.asarray([curve[ib-1], curve[ib], curve[ib+1]], dtype=np.float64)
        try:
            a, b, _c = np.polyfit(rr, ss, 2)
            if a < -1e-12:
                rv = -b / (2.0 * a)
                if min(rr) <= rv <= max(rr):
                    z_refined = 1.0 / rv
                    rec_ref = _evaluate_plane_hypothesis(ref_gray, src_gray, pt_ref, z_refined, best['normal'],
                                                         K_ref, K_src, R_ref_to_src, t_ref_to_src,
                                                         ref_patch, ref_valid, patch_size,
                                                         ref_exclude_patch, src_exclude_mask)
                    if rec_ref is not None and rec_ref['score'] >= best['score'] - 0.02:
                        best = rec_ref
        except (np.linalg.LinAlgError, ValueError, FloatingPointError):
            pass

    return {'m_pt': np.asarray(best['pt_src'], dtype=np.float32),
            'depth': float(best['depth']), 'normal': np.asarray(best['normal'], dtype=np.float64),
            'score': float(best['score']), 'second_score': float(second_score),
            'uniqueness': uniqueness, 'valid_ratio': float(best['valid_ratio']),
            'support_ratio': float(best['support_ratio']),
            'occlusion_ratio': float(best['occlusion_ratio']),
            'center_visible': bool(best['center_visible']),
            'intensity_ncc': float(best['intensity_ncc']),
            'gradient_ncc': float(best['gradient_ncc']),
            'boundary_hit': boundary_hit,
            'X_ref': np.asarray(best['X_ref'], dtype=np.float64),
            'X_src': np.asarray(best['X_src'], dtype=np.float64),
            'reject_reason': None}


def run_localized_plane_sweep_matching_flow(imgA_gray, imgB_gray, u, v, cand, K_L,
                                            left_spec_mask=None, right_spec_mask=None,
                                            enable_fb=LOCAL_PS_ENABLE_FB, left_cache=None):
    """
    Localized plane-sweep / PatchMatch-style matcher for one clicked point.

    Unlike Grad-SIFT interpolation, the clicked pixel itself defines the 3D ray.
    We sweep depth + local surface-normal hypotheses, warp the right image into
    the left patch through the camera geometry, and robustly score the aligned
    appearance.  Local occlusion lowers support/confidence but does not discard
    the candidate frame.
    """
    pt_A = np.array([float(u), float(v)], dtype=np.float32)
    K_R = cand.get('K_R')
    R = cand.get('R_rel')
    t = cand.get('t_rel')
    if K_R is None or R is None or t is None:
        return {'m_pt': None, 'method': '', 'reject_reason': '缺少 K_R / R / T', 'confidence': 0.0}

    z_plane = _estimate_reference_depth_from_plane(pt_A, K_L, cand.get('plane_n'), cand.get('plane_c'))
    if z_plane is None:
        if cand.get('plane_c') is not None:
            z_plane = float(np.asarray(cand['plane_c']).reshape(3)[2])
        else:
            z_plane = 250.0
    z_plane = float(np.clip(z_plane, LOCAL_PS_MIN_DEPTH_MM, LOCAL_PS_MAX_DEPTH_MM))
    base_n = cand.get('plane_n')
    if base_n is None:
        base_n = np.array([0.0, 0.0, 1.0], dtype=np.float64)

    core = _localized_plane_sweep_core(
        imgA_gray, imgB_gray, pt_A, K_L, K_R, R, t,
        center_depth=z_plane, base_normal=base_n,
        ref_exclude_mask=left_spec_mask, src_exclude_mask=right_spec_mask)
    if core.get('m_pt') is None:
        core.update({'method': '', 'confidence': 0.0, 'fb_error': float('inf')})
        return core

    fb_error = float('nan')
    fb_ok = True
    if enable_fb:
        X_R = np.asarray(core['X_src'], dtype=np.float64)
        z_R = float(X_R[2])
        n_R = _normalize_vec(np.asarray(R, dtype=np.float64) @ np.asarray(core['normal'], dtype=np.float64),
                             fallback=[0.0, 0.0, 1.0])
        R_rev = np.asarray(R, dtype=np.float64).T
        t_rev = -R_rev @ np.asarray(t, dtype=np.float64).reshape(3, 1)
        rev = _localized_plane_sweep_core(
            imgB_gray, imgA_gray, core['m_pt'], K_R, K_L, R_rev, t_rev,
            center_depth=z_R, base_normal=n_R,
            ref_exclude_mask=right_spec_mask, src_exclude_mask=left_spec_mask,
            depth_half_range=LOCAL_PS_FB_RANGE_MM,
            coarse_samples=LOCAL_PS_FB_SAMPLES,
            fine_samples=max(7, LOCAL_PS_FB_SAMPLES // 2 * 2 + 1),
            patch_size=LOCAL_PS_PATCH_SIZE,
            normal_tilt_deg=LOCAL_PS_FINE_NORMAL_TILT_DEG,
            fine_normal_tilt_deg=max(1.0, LOCAL_PS_FINE_NORMAL_TILT_DEG * 0.5))
        if rev.get('m_pt') is not None:
            fb_error = float(np.linalg.norm(np.asarray(rev['m_pt'], dtype=np.float32) - pt_A))
            fb_ok = np.isfinite(fb_error)
        else:
            fb_error = float('inf')
            fb_ok = False

    score_term = np.clip((core['score'] - LOCAL_PS_MIN_SCORE) / max(1.0 - LOCAL_PS_MIN_SCORE, 1e-6), 0.0, 1.0)
    uniq_term = np.clip(core['uniqueness'] / max(LOCAL_PS_MIN_UNIQUENESS, 1e-6), 0.0, 1.0)
    valid_term = np.clip((core['valid_ratio'] - LOCAL_PS_MIN_VALID_RATIO) / max(1.0 - LOCAL_PS_MIN_VALID_RATIO, 1e-6), 0.0, 1.0)
    support_term = np.clip((core['support_ratio'] - LOCAL_PS_MIN_SUPPORT_RATIO) / max(1.0 - LOCAL_PS_MIN_SUPPORT_RATIO, 1e-6), 0.0, 1.0)
    center_term = 1.0 if core['center_visible'] else 0.35
    boundary_term = 0.60 if core.get('boundary_hit', False) else 1.0
    if enable_fb:
        fb_term = float(np.exp(-0.5 * (fb_error / max(LOCAL_PS_FB_SIGMA_PX, 1e-6)) ** 2)) if fb_ok else 0.20
    else:
        fb_term = 1.0
    # Keep confidence soft: partial occlusion lowers weight rather than deleting
    # the entire right frame.  Catastrophic photometric/geometric failure is the
    # only case where this matcher returns no point and lets Grad-SIFT fallback.
    confidence = float(np.clip(
        score_term * (0.45 + 0.55 * uniq_term) *
        (0.55 + 0.45 * valid_term) * (0.55 + 0.45 * support_term) *
        center_term * boundary_term * (0.35 + 0.65 * fb_term), 0.0, 1.0))

    catastrophic = (core['score'] < LOCAL_PS_MIN_SCORE or
                    core['valid_ratio'] < LOCAL_PS_MIN_VALID_RATIO or
                    core['support_ratio'] < LOCAL_PS_MIN_SUPPORT_RATIO)
    if catastrophic:
        return {**core, 'm_pt': None, 'method': '',
                'reject_reason': (f"PlaneSweep低可信: score={core['score']:.2f}, "
                                  f"valid={core['valid_ratio']:.2f}, support={core['support_ratio']:.2f}"),
                'confidence': confidence, 'fb_error': fb_error}

    core.update({'method': 'LocalPlaneSweep', 'confidence': confidence,
                 'fb_error': fb_error, 'reject_reason': None,
                 'plane_reference_depth': z_plane})
    print(
        f"   [LocalPlaneSweep] Z={core['depth']:.2f}mm, score={core['score']:.3f}, "
        f"second={core['second_score']:.3f}, unique={core['uniqueness']:.3f}, "
        f"valid={core['valid_ratio']:.2f}, support={core['support_ratio']:.2f}, "
        f"occ={core['occlusion_ratio']:.2f}, center={'Y' if core['center_visible'] else 'N'}, "
        f"FB={core['fb_error']:.2f}px, conf={core['confidence']:.3f}"
    )
    return core
