...
UI_BUILD = "2026-08-26 fixed-camera-v5.9-4k-halfres-fullres-subpix"
VIDEO_FILE_TYPES = [
...
class TwoVideoRTValidationUI(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        self.aruco_outer_edge_refine_var = tk.BooleanVar(value=False)
        self.aruco_four_k_halfres_seed_var = tk.BooleanVar(value=True)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(
            options,
            text="4K 半解析粗定位 → 原圖 SubPix（v5.9，建議開啟）",
            variable=self.aruco_four_k_halfres_seed_var,
        ).grid(row=3, column=0, columnspan=5, sticky="w", pady=(7, 0))
        ttk.Label(
            options,
            text="僅 3840×2160 啟用：bilateral → 1920×1080 ArUco → 座標×2 → 原始 4K gray 用 13×13 cornerSubPix。",
            foreground="#555555",
        ).grid(row=3, column=5, columnspan=10, sticky="w", pady=(7, 0))
        ttk.Checkbutton(
            options,
            text="Outer-edge refine（v5.8，可選）",
            variable=self.aruco_outer_edge_refine_var,
        ).grid(row=4, column=0, columnspan=3, sticky="w", pady=(5, 0))
        ttk.Label(
            options,
            text="預設關閉，方便單獨驗證 v5.9 的 half-res seed + full-res SubPix；需要時仍可疊加 outer-edge。",
            foreground="#555555",
        ).grid(row=4, column=3, columnspan=12, sticky="w", pady=(5, 0))
    ...
    def _collect_inputs(self) -> dict[str, Any]:
        ...
        return {
            ...
            "aruco_four_k_halfres_seed": bool(self.aruco_four_k_halfres_seed_var.get()),
        }
    ...
    def _start_analysis(self) -> None:
        ...
        self._append_log(
            ...
            f"4k_halfres_seed={'ON' if inputs['aruco_four_k_halfres_seed'] else 'OFF'} | "
            f"SIFT=disabled"
        )
    ...
    def _analysis_worker(self, inputs: dict[str, Any]) -> None:
        ...
        try:
            from Algorithm import (
                video_pose_analysis_temporal_unified_pattern_guided_local_window_validation_v5_9
            ...
            result = validation_backend.analyze_two_video_segments(
                ...
                aruco_four_k_halfres_seed=inputs["aruco_four_k_halfres_seed"],
    ...
    def _format_metrics(self, result: dict[str, Any]) -> str:
        ...
        lines.append(
            ...
            f"4K half-res seed: {'ON' if result.get('aruco_four_k_halfres_seed', False) else 'OFF'}"
        )
