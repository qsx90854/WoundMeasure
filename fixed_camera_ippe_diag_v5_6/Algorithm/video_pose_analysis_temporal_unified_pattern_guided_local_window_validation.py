...
class _MarkerDetector:
    ...
    def detect_with_raw(
        ...
        """Return (cornerSubPix-refined corners, raw detectMarkers corners).

        Keeping the raw ArUco corners lets the validation UI quantify whether the
        fixed 5x5 cornerSubPix step introduces a systematic inward/outward bias.
        The refined dictionary remains the one used by the RT estimator.
        """
        ...
        detect_gray = self._clahe.apply(gray) if self._use_clahe else gray
        ...
        refined_result: Dict[int, np.ndarray] = {}
        raw_result: Dict[int, np.ndarray] = {}
        for marker_id, marker_corners in zip(ids.reshape(-1), corners):
            raw = np.asarray(marker_corners, np.float32).reshape(4, 2).copy()
            refined = raw.copy()
            try:
                cv2.cornerSubPix(
            ...
            raw_result[marker_id] = raw
            refined_result[marker_id] = refined
        return refined_result, raw_result

    def detect(self, frame: np.ndarray) -> Dict[int, np.ndarray]:
        refined, _raw = self.detect_with_raw(frame)
        return refined
    ...
    *,
    ...
    def detect(segment: str, local_index: int) -> Dict[int, np.ndarray]:
        ...
        if override is not None:
            ...
            corners, raw_corners = detector.detect_with_raw(
    ...
    try:
        ...
        detector = _MarkerDetector(use_clahe=aruco_use_clahe)
        emit_log(f"ArUco preprocessing: CLAHE={'ON' if aruco_use_clahe else 'OFF'}")
        ...
        emit_log(
            ...
            f"CLAHE={'ON' if aruco_use_clahe else 'OFF'} subpix_window=5x5")
