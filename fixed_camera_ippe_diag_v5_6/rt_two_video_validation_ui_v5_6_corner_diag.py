...
UI_BUILD = "2026-08-20 fixed-camera-v5.6-raw-vs-subpix-corner-diag"
VIDEO_FILE_TYPES = [
...
def draw_marker_overlay_post_scale(
    ...
    """Draw raw/refined corner diagnostics after crop/resize.

    Refined cornerSubPix coordinates are blue ``+`` crosshairs.  Raw
    ...
    legend = " | RAW=yellow x, SUBPIX=blue +" if show_raw_vs_subpix else " | SUBPIX=blue +"
    cv2.putText(
        ...
        if isinstance(per_corner, list):
            for item in per_corner:
                ...
                lines.append(
                    ...
                    f"subpix=({refined[0]:.6f},{refined[1]:.6f}) "
...
def _append_raw_vs_subpix_ippe_lines(lines: list[str], label: str, diag: Any) -> None:
    ...
    for item in comparisons:
        ...
        lines.append(
            f"{label} branch {item.get('branch')}: raw→SubPix pose shift "
            f"rotation={rot:.6f} deg, translation={trans:.6f} mm, "
            f"raw_RMS={raw_rms!r}px, subpix_RMS={subpix_rms!r}px"
    ...
    def _format_metrics(self, result: dict[str, Any]) -> str:
        ...
        if isinstance(diagnostics, dict):
            ...
            if isinstance(corner_refine, dict):
                lines.extend(("", "=== RAW detectMarkers -> cornerSubPix DIAGNOSTICS ==="))
                lines.append(f"cornerSubPix window: {corner_refine.get('subpix_window')}")
            ...
            if isinstance(ippe, dict):
                if isinstance(ippe.get("raw_vs_cornerSubPix_A"), dict) or isinstance(ippe.get("raw_vs_cornerSubPix_B"), dict):
                    lines.extend(("", "=== RAW CORNER vs SubPix IPPE POSE SHIFT ==="))
