...
* an offline beam search that selects a temporally consistent hypothesis path;
...
# probes are opt-in because decoding/detection, not the SE(3) beam search, is the
# measurable cost in this pipeline.
ENABLE_TEMPORAL_NEIGHBOR_PROBES = False
TEMPORAL_NEIGHBOR_RADIUS = 1
TEMPORAL_BEAM_WIDTH = 8
...
TEMPORAL_PAIR_MIN_BASELINE_MARGIN_MM = 2.0
TEMPORAL_BRANCH_PRIOR_MAX_COST = 0.08
...
def _temporal_pose_marker_errors(
    ...
    rvec = cv2.Rodrigues(np.asarray(rotation, np.float64))[0]
    translation = np.asarray(translation, np.float64).reshape(3, 1)
    errors = {}
    depths_ok = {}
    for marker_id, observed in corners_dict.items():
        if marker_id not in marker_map:
            continue
        map_R, map_t = marker_map[marker_id]
        object_reference = (
            np.asarray(map_R, np.float64) @ object_local.T
            + np.asarray(map_t, np.float64).reshape(3, 1)).T
        points_camera = (
            np.asarray(rotation, np.float64) @ object_reference.T + translation).T
        depths_ok[int(marker_id)] = bool(np.all(points_camera[:, 2] > 1e-6))
        projected, _ = cv2.projectPoints(
        ...
        errors[int(marker_id)] = float(
            np.sqrt(np.mean(np.sum(residual * residual, axis=1))))
    return errors, depths_ok
...
def _build_temporal_frame_candidates(
    ...
    candidates = []
    for seed in seeds:
        errors, depths = _temporal_pose_marker_errors(
            seed['R'], seed['t'], corners_dict, marker_map,
            camera_matrix, distortion, marker_size_mm)
        inlier_ids = sorted(
            marker_id for marker_id, error in errors.items()
            if depths.get(marker_id, False)
            and error <= TEMPORAL_MARKER_GROUP_RMS_MAX_PX)
        if not inlier_ids:
            continue
        rotation, translation = _temporal_refine_pose_with_marker_groups(
            seed['R'], seed['t'], inlier_ids, corners_dict, marker_map,
            camera_matrix, distortion, marker_size_mm)
        errors, depths = _temporal_pose_marker_errors(
        ...
        rms = float(np.sqrt(np.mean([
            errors[marker_id] ** 2 for marker_id in inlier_ids
        ])))
        outlier_ids = sorted(set(available_ids) - set(inlier_ids))
        emission = (
            TEMPORAL_REPROJECTION_WEIGHT * rms / math.sqrt(len(inlier_ids))
            + TEMPORAL_OUTLIER_MARKER_PENALTY * len(outlier_ids))
        candidates.append({
            **seed,
            'R': rotation,
            't': translation,
            'camera_center': _temporal_camera_center(rotation, translation),
            'source': 'joint_marker_pnp' if len(inlier_ids) >= 2 else seed['source'],
            'label': (
                f"{seed['label']}->J[{','.join(map(str, inlier_ids))}]"
                if len(inlier_ids) >= 2 else seed['label']),
            'marker_ids': available_ids,
            'inlier_marker_ids': inlier_ids,
            'outlier_marker_ids': outlier_ids,
    ...
    for candidate in sorted(candidates, key=lambda value: (
            value['emission_cost'], value['label'])):
        duplicate = False
        for accepted in deduplicated:
            if (_temporal_rotation_distance_deg(candidate['R'], accepted['R']) < 0.05
                    and np.linalg.norm(
                        candidate['camera_center'] - accepted['camera_center']) < 0.05):
                duplicate = True
                break
        if not duplicate:
            deduplicated.append(candidate)
        if len(deduplicated) >= TEMPORAL_MAX_FRAME_CANDIDATES:
            break
    return deduplicated
...
def _temporal_transition_cost(path, frame_index, candidate):
    if not path:
        return 0.0, {}
    previous_index, previous = path[-1]
    gap = max(1, int(frame_index) - int(previous_index))
    ...
    if len(path) >= 2:
        before_index, before = path[-2]
        before_gap = max(1, int(previous_index) - int(before_index))
        # R is world -> camera.  Form increments from Q=R.T so both
        # angular-velocity vectors are expressed in the fixed world frame
        ...
        center_acceleration = float(np.linalg.norm(
            current_center_velocity - previous_center_velocity))
    cost = (
    ...
    return float(cost), {
        ...
        'rotation_acceleration': float(rotation_acceleration),
        'camera_center_acceleration': float(center_acceleration),
        'transition_cost': float(cost),
    }


def _select_temporal_pose_path(frame_candidates, beam_width=None):
    """Select one pose per observed frame with a bounded offline beam search."""
    beam_width = int(beam_width or TEMPORAL_BEAM_WIDTH)
    available = [
        (int(frame_index), list(candidates))
        for frame_index, candidates in sorted(frame_candidates.items())
        if candidates
    ]
    if not available:
        return {}, {
            'status': 'NO_POSE_CANDIDATES',
            'path_cost': None,
            'second_cost': None,
            'margin': None,
            'frames': {},
    ...
    beams = [{
        'cost': float(candidate['emission_cost']),
        'path': [(available[0][0], candidate)],
    } for candidate in available[0][1]]
    beams.sort(key=lambda beam: (
        beam['cost'], tuple(item[1]['label'] for item in beam['path'])))
    beams = beams[:beam_width]
    for frame_index, candidates in available[1:]:
        expanded = []
        for beam in beams:
            for candidate in candidates:
                transition, _details = _temporal_transition_cost(
                    beam['path'], frame_index, candidate)
                expanded.append({
                    'cost': (
                        beam['cost'] + float(candidate['emission_cost']) + transition),
                    'path': beam['path'] + [(frame_index, candidate)],
                })
        expanded.sort(key=lambda beam: (
            beam['cost'], tuple(item[1]['label'] for item in beam['path'])))
        beams = expanded[:beam_width]
    best = beams[0]
    selected = dict(best['path'])
    second_cost = float(beams[1]['cost']) if len(beams) > 1 else None
    frames = {}
    history = []
    for frame_index, candidate in best['path']:
        _transition, details = _temporal_transition_cost(
            history, frame_index, candidate)
        frames[int(frame_index)] = {
            'chosen_label': candidate['label'],
            'source': candidate['source'],
            'seed_marker_id': candidate['seed_marker_id'],
            'seed_branch': candidate['seed_branch'],
            'candidate_count': len(frame_candidates[frame_index]),
            'inlier_marker_ids': list(candidate['inlier_marker_ids']),
            'outlier_marker_ids': list(candidate['outlier_marker_ids']),
    ...
    return selected, {
        'status': 'OK_TEMPORAL_BEAM_PATH',
        'observation_count': len(best['path']),
        'beam_width': beam_width,
        'path_cost': float(best['cost']),
        'second_cost': second_cost,
        'margin': (
            second_cost - float(best['cost']) if second_cost is not None else None),
        'normalized_margin': (
            (second_cost - float(best['cost'])) / max(len(best['path']), 1)
...
def analyze_video_frames(
    ...
    for stage_idx, num_samples in enumerate(stages):
        ...
        # This is refined below from the *per-frame* usable marker groups.  The
        # union can contain two IDs while an endpoint sees only one, which is
        # exactly where a planar IPPE ambiguity must remain available to SIFT.
        few_marker_mode = (len(shared_ids) == 1)

        # Build one rigid marker map in the reference-marker coordinate system.
        # Every IPPE branch from every temporal probe participates in the robust
        # relation consensus; a single blurred/ambiguous frame can no longer pull
        # the marker map through a plain arithmetic mean.
        ...
        marker_map, marker_map_diagnostics = _build_temporal_marker_map(
            temporal_info, temporal_marker_ids, ref_id, mtx_L, dist_L,
            marker_size_mm)
        ...
        ambiguous_core_indices = sorted(
            ...
            and len(candidate.get('inlier_marker_ids', [])) < 2)
        few_marker_mode = bool(ambiguous_core_indices)
        if few_marker_mode:
            log_and_print(
                "ℹ️ [逐幀單標籤模式] IPPE 時序先驗 + SIFT 可覆寫消歧，"
                f"frames={ambiguous_core_indices}")
        temporal_valid_poses = {
            int(frame_index): (
        ...
        log_and_print(
            ...
            f"path_margin={path_diagnostics.get('normalized_margin')}, "
            f"map_ids={sorted(marker_map)}")

        def build_temporal_valid(core_items):
            valid = []
            for item in core_items:
                candidate = selected_temporal_path.get(int(item['idx']))
                if candidate is None:
                    continue
                ordered_candidates = [candidate]
                # In single-marker planar geometry temporal continuity is a
                # strong prior, not an oracle.  Keep the other IPPE hypothesis
                # behind the selected one so the existing marker+SIFT check can
                # overrule a low-confidence temporal path without another SIFT
                # extraction.  Multi-marker frames keep the joint selected pose.
                if len(candidate.get('inlier_marker_ids', [])) < 2:
                    ordered_candidates.extend(sorted(
                        (alternative for alternative in frame_candidates.get(
                            int(item['idx']), []) if alternative is not candidate),
                        key=lambda alternative: (
                            alternative['emission_cost'], alternative['label'])))
                branches = [
        ...
        pairs = []
        path_margin = path_diagnostics.get('normalized_margin')
        temporal_branch_confidence = float(np.clip(
            0.0 if path_margin is None else path_margin
            / max(TEMPORAL_BRANCH_CONFIDENCE_MARGIN, 1e-9),
            0.0, 1.0))
            ...
            for item_e in valid_end:
                ...
                for bi_s, (R_s, t_s) in enumerate(item_s['branches']):
                    for bi_e, (R_e, t_e) in enumerate(item_e['branches']):
                        ...
                        # MIN_BASELINE_MM is injected by the caller at runtime;
                        # derive this gate dynamically instead of using the stale
                        # import-time PAIR_CANDIDATE_MIN_BASELINE_MM value.
                        pair_candidate_min_baseline = (
                            float(MIN_BASELINE_MM)
                            + float(TEMPORAL_PAIR_MIN_BASELINE_MARGIN_MM))
                        ...
                        branch_deviations = int(bi_s != 0) + int(bi_e != 0)
                        pair_metrics['temporal_branch_selected'] = (
                            branch_deviations == 0)
                        pair_metrics['temporal_branch_confidence'] = (
                            temporal_branch_confidence)
                        pair_metrics['temporal_branch_prior_cost'] = float(
                            TEMPORAL_BRANCH_PRIOR_MAX_COST
                            * temporal_branch_confidence * branch_deviations)
        ...
        for cand_tuple in topk:
            ...
            combined += pair_metrics.get('temporal_branch_prior_cost', 0.0)
