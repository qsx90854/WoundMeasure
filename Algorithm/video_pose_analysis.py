...
# ---- 單標籤模式 (場景僅一個 ArUco pattern) ----
PAIR_EPI_TOPK_SINGLE = 16           # 單標籤: 分支組合較多，擴大特徵極線重排範圍
SINGLE_MARKER_EPI_OK_PX = 0.8       # 單標籤: 配對提前收斂的特徵極線殘差門檻 (px)
SINGLE_MARKER_EPI_EXTRA_PX = 1.5    # 單標籤: 次佳對接受的特徵極線殘差上限 (px)
...
def analyze_video_frames(video_path, start_n, end_n, K_L, dist_L, mtx_L, marker_size_mm, select_mode="average", range_mode="fixed", progress_callback=None):
    ...
    for stage_idx, num_samples in enumerate(stages):
        ...
        few_marker_mode = (len(shared_ids) <= 2)
        if few_marker_mode:
            log_and_print(f"ℹ️ [少標籤模式] 共享標籤 {len(shared_ids)} 個：RT 以 ref 標籤 IPPE 雙分支 + 特徵極線消歧解算；"
                          f"其他標籤僅用於尺度與極線驗證 (不假設標籤共面)")
        ...
        topk_n = PAIR_EPI_TOPK_SINGLE if few_marker_mode else PAIR_EPI_TOPK
        topk = pairs[:topk_n]
        reranked = []
        ...
        pairs = [t for _combined, t in reranked] + pairs[topk_n:]
        ...
        if few_marker_mode:
            stage_ok = feat_epi is not None and feat_epi < SINGLE_MARKER_EPI_OK_PX
        else:
            stage_ok = best_err < 0.2
        if stage_ok:
            ...
            if few_marker_mode:
                log_and_print(f"🎉 第 {stage_idx + 1} 階段搜尋成功！特徵極線殘差 {fe_str} px < {SINGLE_MARKER_EPI_OK_PX} px (IPPE 分支 {best_branch})")
            else:
                log_and_print(f"🎉 第 {stage_idx + 1} 階段搜尋成功！在 {num_samples} 張抽樣下，找到誤差 < 0.2 px 的最佳配對：誤差 {best_err:.3f} px")
            stage_success = True
        else:
            if few_marker_mode:
                log_and_print(f"ℹ️ 第 {stage_idx + 1} 階段：最佳特徵極線殘差 {fe_str} px (門檻 {SINGLE_MARKER_EPI_OK_PX} px)")
            else:
        ...
        if stage_success or num_samples == 50:
            if not stage_success:
                best_start, best_end, R_rel, t_rel, baseline = best_cand[2], best_cand[3], best_cand[4], best_cand[5], best_cand[6]
                best_branch = best_cand[7].get('branch')
                log_and_print(f"⚠️ 達到最大抽樣張數 (50 張) 仍未找到合格配對。降級使用當前最優對 (reproj {best_err:.3f} px)")
                
            # 次佳對選取：重投影誤差小於 0.5 px 且 baseline 大於門檻的其餘配對，最多取 5 組
            candidates_scores = []
            if few_marker_mode:
                # 只從已做特徵極線驗證的前段候選挑，且每個起始影格只取最佳分支
                _seen_extra_idx = set()
                for pair_score, err, item_s, item_e, R_rel_c, t_rel_c, bsl, pair_metrics in pairs[:topk_n]:
                    if item_e['idx'] != best_end['idx'] or item_s['idx'] == best_start['idx']:
                        continue
                    if item_s['idx'] in _seen_extra_idx:
                        continue
                    fe = pair_metrics.get('feat_epi_px')
                    if fe is None or fe >= SINGLE_MARKER_EPI_EXTRA_PX:
                        continue
    ...
    if few_marker_mode and best_branch is not None and 'branches' in best_start:
        ...
        log_and_print(f"ℹ️ [少標籤模式] 依特徵消歧採用 IPPE 分支組合 (s={_bi_s}, e={_bi_e})")
    ...
    if few_marker_mode:
        ...
        if _m_final is not None:
            ...
            log_and_print(f"ℹ️ [少標籤模式] 姿態指標改用特徵極線殘差 {_fe_final:.3f} px "
    ...
    if few_marker_mode:
        # 參考平面只由 ref 標籤 (最小 ID) 定義——標籤可能貼在不規則表面，不可合併多標籤擬合。
        # 以最終 RT 三角化該標籤角點擬合平面，與量測點同一幾何鏈 (誤差相消)。
