...
def analyze_two_video_segments(
    ...
    *,
    ...
    try:
        ...
        for segment in ("A", "B"):
            for local_index in core_indices[segment]:
                corners = detect(segment, local_index)
                if corners:
                    core_infos[segment].append({"idx": int(local_index), "corners": corners})
        if not core_infos["A"] or not core_infos["B"]:
            raise RuntimeError(
                "ArUco was not detected in at least one segment's core probes")
