...
int SiftMatchGPU::_CreateContextGL(LiteWindow* window)
{
	//Create an OpenGL Context?
	__window = window;
    if (__language >= SIFTMATCH_CUDA) {}
	else if(!GlobalUtil::CreateWindowEZ(__window))
	{
...
LiteWindow* SiftMatchGPU::_DestroyContextGL(bool reset_window)
{
	if(reset_window)
	{
		LiteWindow* win = __window;
		__window = NULL;
		return win;
	}
	delete __window;
...
int SiftMatchGPU::GetLanguage() const
{
	return __language;
}
