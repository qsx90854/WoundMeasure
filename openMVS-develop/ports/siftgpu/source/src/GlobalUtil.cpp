...
#ifdef _DEBUG
const char* gluErrorStringReplacement(GLenum err) {
    switch (err) {
...
void GlobalUtil::CheckErrorsGL(const char* location)
{
	...
	{
		errstr = gluErrorStringReplacement(errnum);
		if(errstr) {
			std::cerr << errstr;
		}
		else {
...
void GlobalUtil::InitGLParam(int NotTargetGL)
{
    ...
    }else
    {
	    ...
	    if(vendor)
	    {
			...
			if(GlobalUtil::_IsNvidia && GLAD_GL_NVX_gpu_memory_info)
			{
			...
			}else if(strstr(vendor, "ATI") && GLAD_GL_ATI_meminfo)
			{
    	...
    	if (!GLAD_GL_ARB_fragment_shader || !GLAD_GL_ARB_shader_objects || !GLAD_GL_ARB_shading_language_100)
	    {
		...
		GlobalUtil::_SupportNVFloat = GLAD_GL_NV_float_buffer;
		GlobalUtil::_SupportTextureRG = GLAD_GL_ARB_texture_rg;
...
int GlobalUtil::CreateWindowEZ(LiteWindow*& window)
{
	if(window == NULL) {
		...
		// Prefer headless EGL window if available
		window = new LiteWindowEGL();
#else
		// Default to GLFW window
		window = new LiteWindowGLFW();
#endif
	}
	if(!window->IsValid())
