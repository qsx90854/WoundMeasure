...
public:
    ...
    virtual void* GetProgramID() { return reinterpret_cast<void*>(static_cast<uintptr_t>(_programID)); }
public:
	ProgramGLSL();
	~ProgramGLSL();
...
public:
	...
	std::vector<FilterProgram*>   f_gaussian_skip0_v;
...
public:
	...
	virtual void SetFeatureDescirptorParam(int gtex, int otex, float dwidth, float fwidth, float width, float height, float sigma);
	static void  WriteOrientationCodeToStream(std::ostream& out);
