...
public:
	...
	enum SIFTGPU_LANGUAGE	{
		SIFTGPULANG_GLSL = 0,
		SIFTGPULANG_CUDA = 1,
		SIFTGPULANG_OPENCL = 2
	};
...
public:
	...
	//(Optional) pass in a window created previously to share the context
	SIFTGPU_EXPORT virtual int CreateContextGL(LiteWindow* window = nullptr);
	//destroy the created OpenGL context..
	//(Optional) reset_window: if true, the contained window will be reset to null and returned
	// to detach the window's lifetime from SiftGPU and let the user manage it
	SIFTGPU_EXPORT virtual LiteWindow* DestroyContextGL(bool reset_window = false);
	...
	//get the language used in SiftGPU
	SIFTGPU_EXPORT virtual int GetLanguage() const;
	//set verbose mode
	...
	SIFTGPU_EXPORT virtual int GetImageCount() const;
	...
	SIFTGPU_EXPORT int GetFeatureCountThreshold() const;
	SIFTGPU_EXPORT int GetMaxOrientation() const;
	SIFTGPU_EXPORT int GetMaxDimension() const;
	SIFTGPU_EXPORT int GetMaxNumFeatures() const;
};
...
protected:
	...
	SIFTGPU_EXPORT virtual int  _CreateContextGL(LiteWindow* window = nullptr);
	SIFTGPU_EXPORT virtual LiteWindow*  _DestroyContextGL(bool reset_window = false);
...
public:
	...
	//(Optional) pass in a window created previously to share the context
	inline int  CreateContextGL(LiteWindow* window = nullptr) {return _CreateContextGL(window);}
	//destroy the created OpenGL context..
	//(Optional) reset_window: if true, the contained window will be reset to null and returned
	// to detach the window's lifetime from SiftMatchGPU and let the user manage it
	inline LiteWindow* DestroyContextGL(bool reset_window = false) {return _DestroyContextGL(reset_window);}
	//verify the current opengl context..
	...
	SIFTGPU_EXPORT virtual int  GetSiftMatch(
				int max_match, //length of the match_buffer
