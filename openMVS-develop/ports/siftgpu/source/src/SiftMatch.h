...
public:
  bool Allocate(int max_sift, int mbm) override;
	...
	void SetDescriptors(int index, int num, const unsigned char * descriptor, int id = -1) override;
	void SetDescriptors(int index, int num, const float * descriptor, int id = -1) override;
	void SetFeautreLocation(int index, const float* locatoins, int gap) override;
	int  GetSiftMatch(int max_match, uint32_t match_buffer[][2], float distmax, float ratiomax, int mbm) override;
	int  GetGuidedSiftMatch(int max_match, uint32_t match_buffer[][2], float* H, float* F,
		float distmax, float ratiomax, float hdistmax,float fdistmax, int mbm) override;
};
