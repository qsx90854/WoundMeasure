...
int SiftGPU::CreateContextGL(LiteWindow* window)
{
	_window = window;
    ...
    else if(!GlobalUtil::CreateWindowEZ(_window))
    {
...
LiteWindow* SiftGPU::DestroyContextGL(bool reset_window)
{
	if(reset_window)
	{
		LiteWindow* win = _window;
		_window = NULL;
		return win;
	}
	delete _window;
...
int SiftGPU::GetLanguage() const
{
	if(GlobalUtil::_UseCUDA) return SIFTGPULANG_CUDA;
	if(GlobalUtil::_UseOpenCL) return SIFTGPULANG_OPENCL;
	return SIFTGPULANG_GLSL;
}
...
int SiftGPU::GetImageCount() const
{
	return _list->size();
}
...
int SiftGPU::GetFeatureCountThreshold() const
{
  return GlobalParam::_FeatureCountThreshold;
}

int SiftGPU::GetMaxOrientation() const
{
  return GlobalParam::_MaxOrientation;
}

int SiftGPU::GetMaxDimension() const
{
  return GlobalUtil::_texMaxDim;
}

int SiftGPU::GetMaxNumFeatures() const
{
  return GlobalUtil::_texMaxDimGL;
}
