...
UI_BUILD = "2026-08-19 fixed-camera-v5.3-clahe-toggle"
VIDEO_FILE_TYPES = [
...
def draw_marker_overlay(frame: np.ndarray, corners: Any, caption: str) -> np.ndarray:
    """Draw thin X marks, IDs and corner indices on detected marker corners."""
    ...
    for marker_id, points_f in _iter_marker_corners(corners):
        ...
        for corner_index, (x, y) in enumerate(points):
            ...
            cv2.putText(
                output,
                str(corner_index),
                (x + x_half_size + 2, y - x_half_size - 1),
