    ...
    def compute_measure(u, v, snap_cand, snap_imgA_gray, snap_view_state, manual_match_pt=None):
        ...
        if m_pt is None:
            ...
            if m_pt is None and snap_view_state['grad_sift']:
                ...
                patch_g = snap_imgA_gray[max(0,vi-8):min(snap_imgA_gray.shape[0],vi+9),
                                         max(0,ui-8):min(snap_imgA_gray.shape[1],ui+9)]
                if patch_g.size > 0:
                    ...
                    kpts_inj = [cv2.KeyPoint(float(max(0,ui-8)+px), float(max(0,vi-8)+py), 31.0)
        ...
        while True:
            ...
            try:
                ...
                if res_pose is not None:
                    ...
                    if len(objA) >= 3:
                        c_local = np.mean(objA, axis=0)
                        _, _, Vt = np.linalg.svd(objA - c_local)
                        n_local = Vt[-1]
