"""Batch HBVCAM stereo RT estimation and export the results to Excel.
...
import depth_measure_multi_aruco_sbs_camera_v7_demo_HBVCAM_v11 as hbvcam
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run only HBVCAM F50 stereo RT estimation for every video in a "
    ...
    parser.add_argument(
        "--output",
        help="Output .xlsx path. Default: <folder>/HBVCAM_RT_batch_<timestamp>.xlsx",
    )
    parser.add_argument(
        "--calibration",
...
def choose_folder() -> Path | None:
    ...
    selected = filedialog.askdirectory(title="Select HBVCAM stereo video folder")
    root.destroy()
...
def main() -> int:
    ...
    try:
        mtx_left, dist_left, mtx_right, dist_right, answer_extrinsic = (
            hbvcam.load_hbvcam_calibration(calibration_path)
        )
    ...
    hbvcam.RECORD_SAVE_DIR = str(folder)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = (
        Path(args.output).expanduser()
        if args.output
        else folder / f"HBVCAM_RT_batch_{timestamp}.xlsx"
    )
