...
LEFT_GRADIENT_POINTS_COUNT    = 100                        # 左圖周圍取梯度最高的特徵點數量
RIGHT_GRADIENT_POINTS_COUNT   = 200                        # 右圖周圍取梯度最高的特徵點數量
...
def main():
    ...
    def compute_measure(u, v, snap_cand, snap_imgA_gray, snap_view_state, manual_match_pt=None):
        ...
        nonlocal locked_L, locked_R, current_cand
        cand = snap_cand
                ...
                if m_pt is None and snap_view_state['grad_sift']:
                    if snap_view_state.get('use_improved_matching', False):
                        ...
                        if patch_g.size > 0:
                            ...
                            flat = mag.flatten()
                            idx_g = np.argsort(flat)[-min(len(flat), LEFT_GRADIENT_POINTS_COUNT):]
                            kpts_inj = [cv2.KeyPoint(float(u0_A + px), float(v0_A + py), 31.0)
                            ...
                            if patch_r.size > 0:
                                ...
                                flatr = magr.flatten()
                                idx_gr = np.argsort(flatr)[-min(len(flatr), RIGHT_GRADIENT_POINTS_COUNT):]
                                kpts_r = [cv2.KeyPoint(float(u0_B + px), float(v0_B + py), 31.0)
    ...
    def on_lock_L(event):
        nonlocal live_L, locked_L, locked_L_clean, has_set_L, reset_pose_history
        live_L = not live_L
        if not live_L:
    ...
    def on_lock_R(event):
        nonlocal live_R, locked_R, locked_R_clean, has_set_R, reset_pose_history
        live_R = not live_R
        if not live_R:
            if len(frame_buffer) > 0:
