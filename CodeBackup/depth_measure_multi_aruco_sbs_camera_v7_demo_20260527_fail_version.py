...
RT_DIFF_ROT_THR       = 0.15                       # N張右圖模式下，彼此間RT旋轉向量的L2距離閥值 (弧度)
RT_DIFF_TRANS_THR     = 5.0                       # N張右圖模式下，彼此間RT平移向量的L2距離閥值 (mm)
...
def get_joint_relative_pose(imgA_gray, imgB_gray, K_L, K_R, marker_size_mm, global_plane_n=None, global_plane_c=None, prev_marker_poses=None, prev_rel_pose=None):
    ...
    for mid in shared:
        ...
        # 使用 solvePnPGeneric 與 IPPE 求解標籤平面的可能姿態候選解以解決二義性問題
        ok, rvecs, tvecs, errs = cv2.solvePnPGeneric(
            canon, cA[idxA][0], K_L, np.zeros(5),
    ...
    ok_rel = False
    if len(shared) == 1:
        # 單個標籤，使用 IPPE 並選擇相對旋轉最小（Trace 最大）的解
        ok_rel, rvecs_rel, tvecs_rel, _ = cv2.solvePnPGeneric(
        ...
        # 多標籤聯合，使用常規 solvePnP
        if use_guess_rel:
            ok_rel, rv_rel, tv_rel = cv2.solvePnP(
                objA, imgB, K_R, np.zeros(5),
                rvec=rv_rel_init, tvec=tv_rel_init, useExtrinsicGuess=True
            )
        else:
...
def main():
    ...
    # HUD 風格的文字面板
    depth_text = ax_B.text(0.02, 0.05, "", transform=ax_B.transAxes, color='white', fontweight='bold', fontsize=13, bbox=dict(facecolor='#121212', alpha=0.7, edgecolor='#00FFFF', lw=1))
    ...
    for b in [btn_norm_toggle, btn_grad_toggle]:
    ...
    def calc_worker():
        ...
        while True:
            ...
            try:
                ...
                # 進行 RT 相似性比對與 outlier 剔除 (限 multi3 模式下)
                valid_indices = [i for i, fp in enumerate(frame_poses) if fp['pose_valid']]
                if locked_R_mode == 'multi3' and len(valid_indices) >= 2:
                    degrees = {i: 0 for i in valid_indices}
                    for i in range(len(valid_indices)):
                        for j in range(i+1, len(valid_indices)):
                            idx_i = valid_indices[i]
                            idx_j = valid_indices[j]
                            if is_rt_similar(frame_poses[idx_i], frame_poses[idx_j], RT_DIFF_ROT_THR, RT_DIFF_TRANS_THR):
                                degrees[idx_i] += 1
                                degrees[idx_j] += 1
                    ...
                    if max_degree > 0:
                        for idx in valid_indices:
                            if degrees[idx] == 0:
                                frame_poses[idx]['rejected'] = True
                                frame_poses[idx]['rejected_reason'] = "RT差異太大未採用"
                                print(f"⚠️ 右圖{idx+1} 的外參 RT 差異太大，已被剔除不加入平均。")
                    else:
                        # 全部彼此都不相似
                        for idx in valid_indices:
                            frame_poses[idx]['rejected'] = True
                            frame_poses[idx]['rejected_reason'] = "RT差異太大未採用"
                            print(f"⚠️ 所有右圖外參 RT 均不一致，全部剔除。")
                ...
                for i in range(len(frame_poses)):
                    if frame_poses[i]['pose_valid'] and not frame_poses[i].get('rejected', False):
                        res_pose = raw_res_poses[i]
                        R_r_i, t_r_i, baseline_i, objA_i, shared_i, cA_dict_i, cB_dict_i, curr_marker_poses_i, curr_marker_poses_R_i, rv_r_i = res_pose
                ...
                if len(all_R_r) > 0:
                    ...
                    # 算一下實際採用的幀數與剔除的幀數
                    used_count = len(all_R_r)
                    rejected_indices = [idx+1 for idx, f in enumerate(frame_poses) if f.get('rejected', False)]
                    if rejected_indices:
                        rej_str = f", 剔除右圖{','.join(map(str, rejected_indices))}"
                    else:
                        rej_str = ""
                    
                    pose_info = f"ArUco 定位({used_count}幀平均{rej_str}, Bsl: {baseline_val:.1f}mm, {direction})"
                    ...
                    if locked_R_mode == 'multi3' and len(valid_indices) > 0:
                        pose_info = "ArUco 定位(3幀平均): RT差異太大未採用"
                    else:
                        pose_info = "未偵測到 ArUco"
                    print("❌ 未偵測到共享 ArUco 或被剔除")
                    n_local, c_local = global_plane_n, global_plane_c
                ...
                for i in range(len(imgB_grays)):
                    ...
                    fp = frame_poses[i]
                    f_pose_valid = fp['pose_valid'] and not fp.get('rejected', False)
                    ...
                    if f_pose_valid:
                        ...
                        p_info = "RT差異太大未採用" if fp.get('rejected', False) else "未偵測到 ArUco"
                        cand_i.update({
                            'R_rel': None, 't_rel': None,
                            'F': None,
                            'pose_info': f"ArUco 定位(右圖{i+1}): {p_info}",
                            'cornersA': {},
                            'cornersB': {},
                            'curr_marker_poses': {},
                            'curr_marker_poses_R': {}
